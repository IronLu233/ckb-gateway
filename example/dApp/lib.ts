...
import { WalletGatewaySender } from "ckb-gateway";
...
export async function transfer(options: Options): Promise<string> {
  ...
  const sender = new WalletGatewaySender("http://localhost:1234");

  sender.requestGatewaySignDigest(
    messageForSigning.message,
    helpers.createTransactionFromSkeleton(tx),
    messageForSigning.hashContentExceptRawTx,
    "eth_personal_sign"
  );

  let signedMessage = await new Promise<string>((resolve, reject) => {
    sender.on("SignDigestDone", (payload) => {
      if (payload.success) {
        resolve(payload.signedMessage);
      }
      reject();
    });
  });
